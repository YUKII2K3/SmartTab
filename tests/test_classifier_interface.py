...
def test_sklearn_compatible_estimator(
    estimator: SmartTabClassifier,
    check: Callable[[SmartTabClassifier], None],
) -> None:
...
def test_classifier_in_pipeline(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    """Test that SmartTabClassifier works correctly within a sklearn pipeline."""
    X, y = X_y
...
def test_dict_vs_object_preprocessor_config(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    model_dict = SmartTabClassifier(
        inference_config={"PREPROCESS_TRANSFORMS": [dict_config]},
        n_estimators=2,
        random_state=42,
    )

    model_obj = SmartTabClassifier(
...
def patch_layernorm_no_affine(model: nn.Module) -> None:
    ...
    of the SmartTab model when using PyTorch version 2.1.3. The issue arose
    ...
    transformer model used by `SmartTabClassifier`. This model contains such
...
def test_onnx_exportable_cpu(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    X, y = X_y
    with torch.no_grad():
        classifier = SmartTabClassifier(n_estimators=1, device="cpu", random_state=42)
...
def test_get_embeddings(X_y: tuple[np.ndarray, np.ndarray], data_source: str) -> None:
    ...
    model = SmartTabClassifier(n_estimators=n_estimators, random_state=42)
    model.fit(X, y)
...
def test_pandas_output_config():
    ...
    # Initialize SmartTab
    model = SmartTabClassifier(n_estimators=1, random_state=42)
...
def test_constant_feature_handling(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    # Create a SmartTabClassifier with fixed random state for reproducibility
    model = SmartTabClassifier(n_estimators=2, random_state=42)
    model.fit(X, y)
    ...
    model_with_constants = SmartTabClassifier(n_estimators=2, random_state=42)
...
def test_classifier_with_text_and_na() -> None:
    """Test that SmartTabClassifier correctly handles text columns with NA values."""
    ...
    # Initialize and fit SmartTab on data with text+NA and a column with all NAs
    classifier = SmartTabClassifier(device="cpu", n_estimators=2)
