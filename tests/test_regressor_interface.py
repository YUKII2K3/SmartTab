...
from smarttab.preprocessing import PreprocessorConfig
...
@parametrize_with_checks([SmartTabRegressor(n_estimators=2)])
def test_sklearn_compatible_estimator(
    estimator: SmartTabRegressor,
    check: Callable[[SmartTabRegressor], None],
) -> None:
...
def test_regressor_in_pipeline(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    """Test that SmartTabRegressor works correctly within a sklearn pipeline."""
    X, y = X_y
...
def test_dict_vs_object_preprocessor_config(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    model_dict = SmartTabRegressor(
        inference_config={"PREPROCESS_TRANSFORMS": [dict_config]},
        n_estimators=2,
        random_state=42,
    )

    model_obj = SmartTabRegressor(
...
def test_onnx_exportable_cpu(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    X, y = X_y
    with torch.no_grad():
        regressor = SmartTabRegressor(n_estimators=1, device="cpu", random_state=43)
...
def test_get_embeddings(X_y: tuple[np.ndarray, np.ndarray], data_source: str) -> None:
    ...
    model = SmartTabRegressor(n_estimators=n_estimators)
    model.fit(X, y)
...
def test_overflow():
    ...
    regressor = SmartTabRegressor(n_estimators=1, device="cpu", random_state=42)
...
def test_cpu_large_dataset_warning():
    ...
    # Create a CPU model
    model = SmartTabRegressor(device="cpu")
...
def test_cpu_large_dataset_warning_override():
    ...
    # -- Test overrides
    model = SmartTabRegressor(device="cpu", ignore_pretraining_limits=True)
    model.fit(X_large, y_large)

    # Set environment variable to allow large datasets to avoid RuntimeError
    os.environ["SMARTTAB_ALLOW_CPU_LARGE_DATASET"] = "1"
    try:
        model = SmartTabRegressor(device="cpu", ignore_pretraining_limits=False)
        model.fit(X_large, y_large)
    finally:
        # Clean up environment variable
        os.environ.pop("SMARTTAB_ALLOW_CPU_LARGE_DATASET")
...
def test_pandas_output_config():
    ...
    # Initialize SmartTab
    model = SmartTabRegressor(n_estimators=1, random_state=42)
...
def test_constant_feature_handling(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    ...
    # Create a SmartTabRegressor with fixed random state for reproducibility
    model = SmartTabRegressor(n_estimators=2, random_state=42)
    model.fit(X, y)
    ...
    model_with_constants = SmartTabRegressor(n_estimators=2, random_state=42)
...
def test_constant_target(X_y: tuple[np.ndarray, np.ndarray]) -> None:
    """Test that SmartTabRegressor predicts a constant
