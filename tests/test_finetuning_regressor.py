...
from smarttab.model.bar_distribution import (
    BarDistribution,
    FullSupportBarDistribution,
)
from smarttab.preprocessing import RegressorEnsembleConfig
...
def ft_regressor_instance(request) -> SmartTabRegressor:
...
@pytest.fixture(params=devices)
def std_regressor_instance(request) -> SmartTabRegressor:
    """Provides a basic regressor instance, parameterized by device."""
    device = request.param
    if device == "cuda" and not torch.cuda.is_available():
        pytest.skip("CUDA device requested but not available.")
...
def test_smarttab_regressor_finetuning_loop(
    n_estimators,
    device,
    fit_mode,
    inference_precision,
...
class TestSmartTabPreprocessingInspection(unittest.TestCase):
    def test_finetuning_consistency_preprocessing_regressor(self):
        ...
        reg_standard = SmartTabRegressor(
            n_estimators=n_estimators,
            device="cpu",
            random_state=common_seed,
            fit_mode="fit_preprocessors",  # Example standard mode
        )
        reg_batched = SmartTabRegressor(
            n_estimators=n_estimators,
            device="cpu",
            random_state=common_seed,
