...
from smarttab.utils import meta_dataset_collator
...
def classifier_instance(request) -> SmartTabClassifier:
...
def test_smarttab_classifier_finetuning_loop(
    n_estimators,
    device,
    fit_mode,
    inference_precision,
...
def test_datasetcollectionwithpreprocessing_classification_single_dataset(
    synthetic_data, classifier_instance: SmartTabClassifier
) -> None:
...
def test_datasetcollectionwithpreprocessing_classification_multiple_datasets(
    uniform_synthetic_dataset_collection, classifier_instance: SmartTabClassifier
) -> None:
...
class TestSmartTabClassifierPreprocessingInspection(unittest.TestCase):
    def test_finetuning_consistency_preprocessing_classifier(self):
        ...
        (get_preprocessed_datasets -> fit_from_preprocessed
            -> forward)
        for the SmartTabClassifier by comparing the tensors entering the internal model.
        """
        # --- Test Parameters ---
        test_set_size = 0.3
        common_seed = 42
        ...
        clf_standard = SmartTabClassifier(
            n_estimators=n_estimators,
            device="cpu",
            random_state=common_seed,
        ...
        clf_batched = SmartTabClassifier(
            n_estimators=n_estimators,
            device="cpu",
            random_state=common_seed,
            fit_mode="batched",
