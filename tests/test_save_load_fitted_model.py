...
@pytest.mark.parametrize(
    ...
    [
        (SmartTabRegressor, "regression_data"),
        (SmartTabClassifier, "classification_data_with_categoricals"),
    ],
)
def test_save_load_happy_path(estimator_class, data_fixture, request, tmp_path):
    ...
    model.fit(X, y)
    path = tmp_path / "model.smarttab_fit"
...
def test_saving_unfitted_model_raises_error(regression_data, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="Estimator must be fitted before saving"):
        model.save_fit_state(tmp_path / "model.smarttab_fit")
...
def test_loading_mismatched_types_raises_error(regression_data, tmp_path):
    ...
    with pytest.raises(
        TypeError, match="Attempting to load a 'SmartTabRegressor' as 'SmartTabClassifier'"
    ):
        SmartTabClassifier.load_from_fit_state(path)
