"""A collection of random utilities for the SmartTab models."""
...
from smarttab.misc._sklearn_compat import check_array, validate_data
...
if TYPE_CHECKING:
    ...
    from smarttab.classifier import SmartTabClassifier, XType, YType
...
def _get_embeddings(
    model: SmartTabClassifier | SmartTabRegressor,
    X: XType,
    data_source: Literal["train", "test"] = "test",
) -> np.ndarray:
    """Extract embeddings from a fitted SmartTab model.
    ...
    from smarttab.preprocessing import ClassifierEnsembleConfig, RegressorEnsembleConfig
...
def validate_Xy_fit(
    X: XType,
    y: YType,
    estimator: SmartTabRegressor | SmartTabClassifier,
    *,
    ...
    if X.shape[1] > max_num_features and not ignore_pretraining_limits:
        raise ValueError(
            ...
            "supported by the SmartTab model. Set `ignore_pretraining_limits=True` "
    ...
    if X.shape[0] > max_num_samples and not ignore_pretraining_limits:
        raise ValueError(
            ...
            f" by SmartTab. Set `ignore_pretraining_limits=True` to override this "
            f"error!",
        )
