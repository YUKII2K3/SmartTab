"""Module that defines different ways to run inference with SmartTab."""
...
from smarttab.model.memory import MemoryUsageEstimator
from smarttab.preprocessing import fit_preprocessing

if TYPE_CHECKING:
    from smarttab.model.preprocessing import SequentialFeatureTransformer
    from smarttab.model.transformer import PerFeatureTransformer
    from smarttab.preprocessing import EnsembleConfig


@dataclass
class InferenceEngine(ABC):
    """These define how SmartTab inference can be run.
    ...
    def use_torch_inference_mode(self, *, use_inference: bool):
        ...
        `SmartTabClassifier.predict_proba_from_preprocessed` (for batched engine) and
        `SmartTabRegressor.forward` (for batched & fit_preprocessors engines)
