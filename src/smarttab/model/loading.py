...
from smarttab.inference import InferenceEngine
from smarttab.model.bar_distribution import BarDistribution, FullSupportBarDistribution
from smarttab.model.config import ModelConfig
from smarttab.model.encoders import (
    CategoricalEncoder,
    NumericalEncoder,
    OrdinalEncoder,
...
if TYPE_CHECKING:
    ...
    from smarttab import SmartTabClassifier, SmartTabRegressor
...
class ModelSource:
    ...
    def get_classifier_v2(cls) -> ModelSource:
        filenames = [
            "smarttab-v2-classifier.ckpt",
            "smarttab-v2-classifier-gn2p4bpt.ckpt",
            "smarttab-v2-classifier-llderlii.ckpt",
            "smarttab-v2-classifier-od3j1g5m.ckpt",
            "smarttab-v2-classifier-vutqq28w.ckpt",
            "smarttab-v2-classifier-znskzxi4.ckpt",
            "smarttab-v2-classifier-finetuned-zk73skhh.ckpt",
        ]
        return cls(
            repo_id="YUKII2K3/SmartTab-v2-clf",
            default_filename="smarttab-v2-classifier-finetuned-zk73skhh.ckpt",
            filenames=filenames,
        )
    ...
    def get_regressor_v2(cls) -> ModelSource:
        filenames = [
            "smarttab-v2-regressor.ckpt",
            "smarttab-v2-regressor-09gpqh39.ckpt",
            "smarttab-v2-regressor-2noar4o2.ckpt",
            "smarttab-v2-regressor-5wof9ojf.ckpt",
            "smarttab-v2-regressor-wyl4o83o.ckpt",
        ]
        return cls(
            repo_id="YUKII2K3/SmartTab-v2-reg",
            default_filename="smarttab-v2-regressor.ckpt",
            filenames=filenames,
        )
...
def download_model(
    ...
    *,
    ...
    """Download a SmartTab model, trying all available sources.
...
def _user_cache_dir(platform: str, appname: str = "smarttab") -> Path:
    use_instead_path = (Path.cwd() / ".smarttab_models").resolve()
    ...
    if not model_path.exists():
        ...
        if res != "ok":
            ...
            raise RuntimeError(
                ...
                f"https://huggingface.co/YUKII2K3/SmartTab-v2-{repo_type}/resolve/main/{model_name}\n\n"
...
def resolve_model_path(
    ...
    if model_path is None:
        ...
        USER_SMARTTAB_CACHE_DIR_LOCATION = os.environ.get("SMARTTAB_MODEL_CACHE_DIR", "")
        if USER_SMARTTAB_CACHE_DIR_LOCATION.strip() != "":
            model_dir = Path(USER_SMARTTAB_CACHE_DIR_LOCATION)
        else:
            model_dir = _user_cache_dir(platform=sys.platform, appname="smarttab")
...
def save_smarttab_model(
    model: SmartTabRegressor | SmartTabClassifier, save_path: Path | str
) -> None:
    """Save the underlying SmartTab foundation model to ``save_path``.

    This function saves the internal model object of a SmartTab estimator. It does
    **not** store a fitted :class:`SmartTabRegressor`/``Classifier`` instance.

    Args:
        model: The SmartTab estimator whose internal model should be saved.
...
def save_fitted_smarttab_model(estimator: BaseEstimator, path: Path | str) -> None:
    """Persist a fitted SmartTab estimator to ``path``.
    ...
    path = Path(path)
    if path.suffix != ".smarttab_fit":
        raise ValueError("Path must end with .smarttab_fit")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shutil.make_archive(str(path).replace(".smarttab_fit", ""), "zip", tmp)
        shutil.move(str(path).replace(".smarttab_fit", "") + ".zip", path)
...
def load_fitted_smarttab_model(
    path: Path | str, *, device: str | torch.device = "cpu"
) -> BaseEstimator:
    """Load a fitted SmartTab estimator saved with ``save_fitted_smarttab_model``."""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if saved_cls_name == "SmartTabClassifier":
            cls = import_module("smarttab.classifier").SmartTabClassifier
        elif saved_cls_name == "SmartTabRegressor":
            cls = import_module("smarttab.regressor").SmartTabRegressor
        else:
