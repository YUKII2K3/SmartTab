"""Utilities for SmartTab model finetuning processes."""
...
from smarttab.base import ClassifierModelSpecs, RegressorModelSpecs
...
def clone_model_for_evaluation(
    original_model: SmartTabClassifier | SmartTabRegressor,
    eval_init_args: dict,
    model_class: type[SmartTabClassifier | SmartTabRegressor],
) -> SmartTabClassifier | SmartTabRegressor:
    ...
    Args:
        ...
        (SmartTabClassifier or SmartTabRegressor).
        eval_init_args: Initialization arguments for
        the evaluation model instance.
        model_class: The class type (SmartTabClassifier
        or SmartTabRegressor) to instantiate.
    ...
    if hasattr(original_model, "model_") and original_model.model_ is not None:
        ...
        model_spec_obj = None
        if isinstance(original_model, SmartTabClassifier):
            model_spec_obj = ClassifierModelSpecs(
                model=new_model_state,
                config=new_config,
            )
        elif isinstance(original_model, SmartTabRegressor):
